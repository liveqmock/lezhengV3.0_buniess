...
layui.config({
    ...
    $('.js_userinfo').on('click', function () {
        ...
        //给个人信息赋值
        var user = JSON.parse(localStorage.getItem('businessUser'));
        if (!user.imgurl) {
            $("#headimguser").attr("src", "images/0.jpg")
        }else{
            $("#headimguser").attr("src", user.imgurl)
        }
        $(".username").html(user.username);
        $("#staffmumber").html(user.number);
        $("#staffname").html(user.name);
        if (parseInt(user.gender) == 0) {
            $("#gender").html('女');
        } else {
            $("#gender").html('男');
        }
        $("#idcardno").html(user.idcardnum);
    ...
    // 修改密码
    $(document).on('click', '.js_changePwd', function () {
        var user = JSON.parse(localStorage.getItem('businessUser'));
        $(".username").html(user.username);
        layer.open({
            title: '修改密码',
            ...
            btn1: function (index, layero) {
                ...
                var businessusersid = user.businessusersid;
                ...
                if (oldpwd != user.password) {
                    ...
                    //请求修改密码的接口
                    var repwddata = {"businessusersid": businessusersid, "password": oldpwd, "newpassword": newpwd};
                    //console.log(repwddata);
                    $.post('http://192.168.0.249:8080/api/bossApi/business/businessUserChangePW', repwddata, function (data) {
                        //console.log(data);
                        if (data.code == 1) {
